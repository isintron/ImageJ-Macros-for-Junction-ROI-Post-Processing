...
![Example 1](https://github.com/isintron/ImageJ-Macros-for-Junction-ROI-Post-Processing/blob/main/Figures/002.png)

**(a)** Left: Raw skeletonized ROIs exported from TA. Center: ROIs colored in yellow and show Feret diameter (red lines, generated by [IJ Macro "drawFeretsDiameter"](https://imagej.net/ij/macros/FeretsDiameter.txt)). Right: Filtering result. **(b)** Feret Diameter Example.
...
![Example 2](https://github.com/isintron/ImageJ-Macros-for-Junction-ROI-Post-Processing/blob/main/Figures/003.png)
Filtering performance on a real epithelial dataset. (a) Overlay of junction ROIs before (Raw input) and after (Filtered) applying the length and curvature thresholds. Red arrowheads indicate example vertex fragments removed by filtering. (b) Distributions of length (top) and curvature (bottom) across all ROIs, with applied thresholds shown as dashed lines.

Test data are provided in the test_data/ folder, including a scrawl image and a real epithelial image ("Ex_GAP43", plasma membrane labeled with GAP43-mCherry).
